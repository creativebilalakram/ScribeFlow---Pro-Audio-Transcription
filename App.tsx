...
const App: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!file.type.startsWith('audio/')) {
        setError("Invalid format. Audio required.");
        return;
      }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-[#f8f9fb] relative selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-6 flex flex-col items-center justify-center relative z-10 py-10 sm:py-20">
        {status === AppStatus.IDLE || status === AppStatus.RECORDING || status === AppStatus.PAUSED ? (
          <div className="w-full flex flex-col items-center gap-10 sm:gap-20 text-center reveal">
            <div className="space-y-4 sm:space-y-8 max-w-4xl">
              <div className="inline-flex items-center gap-3 px-5 py-2 bg-blue-50 border border-blue-100 rounded-full mx-auto">
                <span className="w-2 h-2 rounded-full bg-blue-600 animate-pulse" />
                <span className="text-[11px] font-black text-blue-600 uppercase tracking-[0.3em]">Agency Intelligence Enabled</span>
              </div>
              <h2 className="text-5xl sm:text-8xl font-black tracking-tight text-zinc-950 leading-[0.9]">
            ...
            <div className="w-full max-w-3xl flex flex-col items-center gap-8 sm:gap-12">
              <div className="relative inline-flex p-1 bg-white shadow-xl rounded-[24px] border border-zinc-100">
                <div 
                  className={`absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-zinc-950 rounded-[20px] shadow-2xl transition-transform duration-500 ${
                    inputMode === 'upload' ? 'translate-x-0' : 'translate-x-full'
                  }`}
                />
                <button onClick={() => setInputMode('upload')} className={`relative z-10 px-8 sm:px-12 py-3.5 text-[13px] font-black transition-colors duration-500 ${inputMode === 'upload' ? 'text-white' : 'text-zinc-400'}`}>File Upload</button>
                <button onClick={() => setInputMode('record')} className={`relative z-10 px-8 sm:px-12 py-3.5 text-[13px] font-black transition-colors duration-500 ${inputMode === 'record' ? 'text-white' : 'text-zinc-400'}`}>Live Capture</button>
              </div>
                ...
                {inputMode === 'upload' ? (
                  <label className="premium-container block cursor-pointer w-full group h-[340px] sm:h-[480px]">
                    ...
                    <div className="inner-content flex flex-col items-center justify-center gap-10 p-8">
                      <div className="w-20 h-20 sm:w-28 sm:h-28 bg-white border border-zinc-100 rounded-[36px] flex items-center justify-center shadow-xl group-hover:bg-zinc-950 group-hover:text-white transition-all duration-500">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-[0.3em]">WAV, MP3, WEBM</p>
                      </div>
                    </div>
                  </label>
                ) : (
          ...
          <div className="w-full max-w-xl flex flex-col items-center gap-12 text-center reveal">
             ...
             <div className="space-y-4">
                <h3 className="text-3xl font-black text-zinc-950">{status === AppStatus.UPLOADING ? 'Ingesting Sequence' : 'Synthesizing...'}</h3>
